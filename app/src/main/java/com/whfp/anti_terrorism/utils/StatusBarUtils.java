 ...
 * 注：需添加依赖implementation files('libs/systembartint-1.0.4.jar')，使用方法；
 * 1.顶部为图片
 * StatusBarUtils.with(this).init();
 * 2.顶部正常布局
 * StatusBarUtils.with(this).setColor(getResources().getColor(R.color.blue))//(这里可选设置图片或者颜色).setDrawable(getResources().getDrawable(R.drawable.shape)).init();
 * 3.顶部为ActionBar
 * StatusBarUtils.with(this).setIsActionBar(true).clearActionBarShadow().setDrawable(getResources().getDrawable(R.drawable.shape)).init();
 * 4.顶部为侧滑菜单（NavigationView）
...
//                attributes.flags |= flagTranslucentStatus;
     ...
     * @param mStatusBar
     */
    public static void setStatusBarParams(Activity mActivity ,View mStatusBar){
